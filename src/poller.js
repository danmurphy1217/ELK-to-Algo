...
class Reader {
  ...
  constructor(location, keysToFetch = null, {
    headers,
    params,
    body
  } = {}) {
  ...
  async read() {
    ...
    let response;
    response = await this._pingEndpoint(this.location)
  ...
  async _pingEndpoint() {
    /**
     * ping `this.location` and return JSON data for top-transactions.
     */

    let json;

    try {
      let res = await fetch(this.location, {
...
class Uploader {
  ...
  async uploadTo(client, {indexName, mappings}) {
    /**
...
const reader = new Reader("http://127.0.0.1:8080/v2/transactions/pending", null, {
  headers: {
    "X-Algo-API-Token": "1e506580e964a022db4a5eb64e561240718afa6bd65e9ef1d5a2f72fe62f3775"
  }
})
...
async function run() {
  ...
  let structure = await read.structure();
  console.log(structure);
  await structure.uploadTo(elasticsearchClient, {mappings: mappings, indexName: "algorand"})
}
